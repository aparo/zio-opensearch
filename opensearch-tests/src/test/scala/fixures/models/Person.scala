...
import zio.schema.opensearch.annotations.{ IndexName, IndexRequireType, Keyword, PK }
...
object Person {
  ...
  implicit val osSchema: OpenSearchSchema[Person] = OpenSearchSchema.gen[Person]
...
object PersonInIndex {
  ...
  implicit val osSchema: OpenSearchSchema[PersonInIndex] = OpenSearchSchema.gen[PersonInIndex]
