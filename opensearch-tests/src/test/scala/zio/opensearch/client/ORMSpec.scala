...
trait ORMSpec {
  ...
  def ormBulker = zio.test.test("orm bulker check") {
    import zio.opensearch.mappings._
    for {
...
//      _                                         <- PersonInIndex.osHelper.drop().ignore
//      _                                         <- OpenSearchSchemaManagerService.createMapping[PersonInIndex].ignore
//      persons                                    = 1.to(100).map(i => PersonInIndex(s"test$i", "TestUser", "TestSurname", Some(i))).toList
//      fork1                                     <- PersonInIndex.osHelper.createMany(persons, skipExisting = true).fork
//      firstCreate                               <- fork1.join
//      _                                         <- PersonInIndex.osHelper.refresh()
//      secondCreate                              <- PersonInIndex.osHelper.createMany(persons, skipExisting = true)
...
//    import zio.opensearch.mappings._
//    for {
//      mapping <- OpenSearchSchemaManagerService.getMapping[Person]
//      indexCreationResponse <- OpenSearchSchemaManagerService.createMapping[Person]
