...
import zio.opensearch.common.bulk.{ BulkActionRequest, BulkResponse, Bulker }
...
import zio.opensearch.common.search._
import zio.opensearch.common.update.{ UpdateRequest, UpdateResponse }
import zio.opensearch.queries.Query
import zio.opensearch.sort.Sort
import zio.schema.opensearch.annotations.{ CustomId, CustomIndex }
import zio.stream.{ ZSink, ZStream }
trait OpenSearchService extends CommonManager {
  def httpService: OpenSearchHttpService

  def config: OpenSearchConfig
...
//  lazy val dirty = Ref.make(false)
  ...
  def getSequenceValue(
    id: String,
    index: String = OpenSearchConstants.SEQUENCE_INDEX
  ...
  def resetSequence(id: String): ZIO[Any, FrameworkException, Unit] =
    this.delete(OpenSearchConstants.SEQUENCE_INDEX, id).unit
  ...
  private[opensearch] lazy val bulker =
...
object OpenSearchService {
  ...
  val live: ZLayer[OpenSearchHttpService, Nothing, OpenSearchService] =
    ZLayer {
      for {
        httpServiceBase <- ZIO.service[OpenSearchHttpService]
        dirtyBulk <- Ref.make(false)
      } yield new OpenSearchService {
        override def httpService: OpenSearchHttpService = httpServiceBase

        override def config: OpenSearchConfig = httpService.openSearchConfig

        override def applicationName: DateMath = httpService.openSearchConfig.applicationName.getOrElse("default")
