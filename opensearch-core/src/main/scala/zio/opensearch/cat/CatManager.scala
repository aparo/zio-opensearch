...
import zio.opensearch.cat.aliases.AliasesRequest
import zio.opensearch.cat.aliases.AliasesResponse
import zio.opensearch.cat.allocation.AllocationRequest
import zio.opensearch.cat.allocation.AllocationResponse
import zio.opensearch.cat.component_templates.ComponentTemplatesRequest
import zio.opensearch.cat.component_templates.ComponentTemplatesResponse
import zio.opensearch.cat.count.CountRequest
import zio.opensearch.cat.count.CountResponse
import zio.opensearch.cat.fielddata.FielddataRequest
import zio.opensearch.cat.fielddata.FielddataResponse
import zio.opensearch.cat.health.HealthRequest
import zio.opensearch.cat.health.HealthResponse
import zio.opensearch.cat.help.HelpRequest
import zio.opensearch.cat.help.HelpResponse
import zio.opensearch.cat.indices.IndicesRequest
import zio.opensearch.cat.indices.IndicesResponse
import zio.opensearch.cat.master.MasterRequest
import zio.opensearch.cat.master.MasterResponse
import zio.opensearch.cat.ml_data_frame_analytics.MlDataFrameAnalyticsRequest
import zio.opensearch.cat.ml_data_frame_analytics.MlDataFrameAnalyticsResponse
import zio.opensearch.cat.ml_datafeeds.MlDatafeedsRequest
import zio.opensearch.cat.ml_datafeeds.MlDatafeedsResponse
import zio.opensearch.cat.ml_jobs.MlJobsRequest
import zio.opensearch.cat.ml_jobs.MlJobsResponse
import zio.opensearch.cat.ml_trained_models.MlTrainedModelsRequest
import zio.opensearch.cat.ml_trained_models.MlTrainedModelsResponse
import zio.opensearch.cat.nodeattrs.NodeattrsRequest
import zio.opensearch.cat.nodeattrs.NodeattrsResponse
import zio.opensearch.cat.nodes.NodesRequest
import zio.opensearch.cat.nodes.NodesResponse
import zio.opensearch.cat.pending_tasks.PendingTasksRequest
import zio.opensearch.cat.pending_tasks.PendingTasksResponse
import zio.opensearch.cat.plugins.PluginsRequest
import zio.opensearch.cat.plugins.PluginsResponse
import zio.opensearch.cat.recovery.RecoveryRequest
import zio.opensearch.cat.recovery.RecoveryResponse
import zio.opensearch.cat.repositories.RepositoriesRequest
import zio.opensearch.cat.repositories.RepositoriesResponse
import zio.opensearch.cat.segments.SegmentsRequest
import zio.opensearch.cat.segments.SegmentsResponse
import zio.opensearch.cat.shards.ShardsRequest
import zio.opensearch.cat.shards.ShardsResponse
import zio.opensearch.cat.snapshots.SnapshotsRequest
import zio.opensearch.cat.snapshots.SnapshotsResponse
import zio.opensearch.cat.tasks.TasksRequest
import zio.opensearch.cat.tasks.TasksResponse
import zio.opensearch.cat.templates.TemplatesRequest
import zio.opensearch.cat.templates.TemplatesResponse
import zio.opensearch.cat.thread_pool.ThreadPoolRequest
import zio.opensearch.cat.thread_pool.ThreadPoolResponse
import zio.opensearch.cat.transforms.TransformsRequest
import zio.opensearch.cat.transforms.TransformsResponse

object CatManager {
  lazy val live: ZLayer[OpenSearchHttpService, Nothing, CatManager] =
    ZLayer {
      for {
...
trait CatManager {
   ...
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/master/cat-alias.html
   *
   ...
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/master/cat-compoentn-templates.html
   *
   * @param format a short version of the Accept header, e.g. json, yaml
   * @param h Comma-separated list of column names to display
   * @param help Return help information
   ...
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/master/cat-fielddata.html
   *
   * @param local If `true`, the request computes the list of selected nodes from the
   * local cluster state. If `false` the list of selected nodes are computed
   * from the cluster state of the master node. In both cases the coordinating
   ...
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/master/cat-health.html
   *
   * @param local If `true`, the request computes the list of selected nodes from the
   * local cluster state. If `false` the list of selected nodes are computed
   * from the cluster state of the master node. In both cases the coordinating
...
//      help = help,
   ...
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/master/cat-master.html
   *
   * @param format a short version of the Accept header, e.g. json, yaml
   * @param h Comma-separated list of column names to display
   * @param help Return help information
   ...
   * For more info refers to http://www.elastic.co/guide/en/opensearch/reference/current/cat-dfanalytics.html
   *
   * @param local If `true`, the request computes the list of selected nodes from the
   * local cluster state. If `false` the list of selected nodes are computed
   * from the cluster state of the master node. In both cases the coordinating
   ...
   * For more info refers to http://www.elastic.co/guide/en/opensearch/reference/current/cat-datafeeds.html
   *
   * @param local If `true`, the request computes the list of selected nodes from the
   * local cluster state. If `false` the list of selected nodes are computed
   * from the cluster state of the master node. In both cases the coordinating
   ...
   * For more info refers to http://www.elastic.co/guide/en/opensearch/reference/current/cat-anomaly-detectors.html
   *
   * @param local If `true`, the request computes the list of selected nodes from the
   * local cluster state. If `false` the list of selected nodes are computed
   * from the cluster state of the master node. In both cases the coordinating
   ...
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/current/cat-trained-model.html
   *
   * @param local If `true`, the request computes the list of selected nodes from the
   * local cluster state. If `false` the list of selected nodes are computed
   * from the cluster state of the master node. In both cases the coordinating
   ...
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/master/cat-plugins.html
   *
   * @param format a short version of the Accept header, e.g. json, yaml
   * @param h Comma-separated list of column names to display
   * @param help Return help information
   ...
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/master/cat-segments.html
   *
   * @param index
...
By default the active, queue and rejected statistics are returned for all thread pools.
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/master/cat-thread-pool.html
   *
   * @param format a short version of the Accept header, e.g. json, yaml
   * @param h Comma-separated list of column names to display
   * @param help Return help information
   ...
   * For more info refers to https://www.elastic.co/guide/en/opensearch/reference/current/cat-transforms.html
   *
   * @param local If `true`, the request computes the list of selected nodes from the
   * local cluster state. If `false` the list of selected nodes are computed
   * from the cluster state of the master node. In both cases the coordinating
