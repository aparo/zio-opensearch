...
final case class InMemorySchemaService(_schemas: Ref[Map[String, OpenSearchSchema[_]]]) extends SchemaService {
  ...
  override def getSchema(
    name: String
  )(implicit authContext: AuthContext): ZIO[Any, FrameworkException, OpenSearchSchema[_]] =
    for {
   ...
   * @return
   */
  override def schemas(implicit authContext: AuthContext): ZIO[Any, FrameworkException, List[OpenSearchSchema[_]]] =
