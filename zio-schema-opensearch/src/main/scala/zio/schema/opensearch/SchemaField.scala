...
sealed trait SchemaField {
  ...
  private def mergeIndexingProperties(
    ...
    if (first == second) Right(first)
      ...
      (first.os_type, second.os_type) match {
        case (OSType.none, _) => Right(second)
        case (_, OSType.none) => Right(first)
        case (OSType.long, a) =>
          a match {
            case OSType.integer | OSType.long | OSType.short | OSType.byte => Right(first)
            case OSType.none                                               => Right(second)
            case _ =>
              Left(
                MergeSchemaException(
                  s"Unable to merge ${first.os_type} - ${second.os_type}",
