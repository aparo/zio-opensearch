...
import zio.schema.opensearch.annotations.{ IndexName, KeyField, KeyManagement, KeyPostProcessing }
...
import zio.schema.opensearch.SchemaNames._
...
/**
 * A OpenSearchSchema rappresentation
 * @param name
 *   name of the OpenSearchSchema
 * @param module
 *   module of the OpenSearchSchema
 * @param `type`
 *   type of the OpenSearchSchema
 * @param version
 *   version of OpenSearchSchema
 * @param description
 *   the description of the OpenSearchSchema
 * @param active
 *   if this entity is active
 * @param labels
 *   a list of labels associated to the OpenSearchSchema
 * @param creationDate
 *   the creation date of the OpenSearchSchema
 * @param creationUser
 *   the reference of the user that created the OpenSearchSchema
 * @param modificationDate
 *   the modification date of the OpenSearchSchema
 * @param modificationUser
 *   the reference of last user that changed the OpenSearchSchema
 * @param key
 ...
 *   the sub fields
 */
final case class OpenSearchSchema[T](
  name: String,
  module: String,
  version: Int = 1,
  `type`: String = "object",
  ...
  def validate(): Either[FrameworkException, OpenSearchSchema[T]] = {
    val iterator = delta.iterator
    var result: Either[FrameworkException, OpenSearchSchema[T]] = Right(this)
   ...
   */
  def merge(
    otherSchema: OpenSearchSchema[T],
    onlyExistsInFirst: Boolean = false
  ): Either[MergeSchemaException, OpenSearchSchema[T]] = {
...
object OpenSearchSchema {
  ...
  def gen[T](implicit schema: Schema[T], decoder: JsonDecoder[T], encoder: JsonEncoder[T]): OpenSearchSchema[T] =
    schemaFromRecord(schema.asInstanceOf[Schema.Record[T]])

  private def schemaFromRecord[T](
    record: Schema.Record[T]
  )(implicit schema: Schema[T], decoder: JsonDecoder[T], encoder: JsonEncoder[T]): OpenSearchSchema[T] = {
    var name = "empty"
    var module = "empty"
    record.id match {
  ...
  )(implicit decoder: JsonDecoder[T], encoder: JsonEncoder[T]): Either[FrameworkException, OpenSearchSchema[T]] =
    zschema match {
...
//  implicit val jsonDecoder: JsonDecoder[OpenSearchSchema] = DeriveJsonDecoder.gen[OpenSearchSchema]
//  implicit val jsonEncoder: JsonEncoder[OpenSearchSchema] = DeriveJsonEncoder.gen[OpenSearchSchema]
}
