...
package zio.opensearch.index

import zio.opensearch.SpecHelper
import zio.opensearch.responses.indices.IndicesGetTemplateResponse
...
class IndexResponsesSpec extends AnyFlatSpec with Matchers with SpecHelper {
  ...
  "Index templates" should "correctly deserialized" in {
    val json = readResourceJSON("/zio/opensearch/index/templates.json")
