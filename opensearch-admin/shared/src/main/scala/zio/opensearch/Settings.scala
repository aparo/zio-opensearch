...
import zio.opensearch.responses.cluster.{ Analysis, ClusterIndexSetting }
import zio.json._
...
object Settings {
  ...
  lazy val SingleShard = (for {
    json <- readResourceJSON("/zio/opensearch/settings.json")
    value <- json.as[Settings]
  } yield value).toOption.getOrElse(Settings())
  def OpenSearchBase: Settings = SingleShard
  def OpenSearchTestBase: Settings = {
