...
import zio.opensearch.ClusterService
import zio.opensearch.orm.{ QueryBuilder, TypedQueryBuilder }
import zio.opensearch.responses.{ HitResponse, ResultDocument, SearchResponse }
import zio.json._
  ...
  def processStep(state: StreamState): ZIO[Any, FrameworkException, (List[HitResponse], Option[StreamState])] = {
    ...
    def getResponse() =
      if (state.scrollId.nonEmpty) {
        client.baseOpenSearchService.searchScroll(state.scrollId.get, keepAlive = "5m")
