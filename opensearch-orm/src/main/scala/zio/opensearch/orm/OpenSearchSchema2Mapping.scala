...
import zio.opensearch.indices.put_index_template.IndexTemplateMapping
import zio.opensearch.indices.requests.PutIndexTemplateRequestBody
import zio.opensearch.mappings._
import zio.exception.InvalidValueException
import zio.schema.opensearch._
object OpenSearchSchema2Mapping {

  def toRootMapping[T](schema: OpenSearchSchema[T]): RootDocumentMapping =
    RootDocumentMapping(properties = schema.fields.map(f => f.name.toLowerCase() -> getMapping(f)).toMap)

  def toIndexTemplate[T](schema: OpenSearchSchema[T]): PutIndexTemplateRequestBody = {
    val documentMapping =
      RootDocumentMapping(properties = schema.fields.map(f => f.name.toLowerCase() -> getMapping(f)).toMap)
