...
import zio.opensearch.queries.{ ExistsQuery, Query }
import zio.json.ast._
import zio.{ Chunk, Ref, ZIO, ZLayer }

object MappingManager {
  lazy val live: ZLayer[OpenSearchService with IndicesManager with ClusterManager, Nothing, MappingManager] =
    ZLayer {
      for {
  ...
  private def computeColumnsCardinality(index: String, columns: Iterable[String])(
    ...
    ZIO.collectAll {
      Chunk.fromIterable(columns.map { col =>
        for {
          countResponse <- openSearchService.count(
            indices = Chunk(index),
