...
import zio.opensearch.aggregations.{ Aggregation, ComposedAggregation, TermsAggregation }
...
import zio.opensearch.search.QueryUtils
...
case class TypedQueryBuilder[T](
  ...
  def getLastUpdate[T: JsonDecoder](field: String): ZIO[Any, FrameworkException, Option[T]] = {
    ...
    //    implicit val client = authContext.opensearch
  ...
  def updateFromBody(json: Json): TypedQueryBuilder[T] = {
    ...
    cursor.getOption[Int]("size").foreach { size =>
      if (size > -1)
        qb = qb.copy(size = Math.min(size, OpenSearchConstants.MAX_RETURNED_DOCUMENTS))
    }
  ...
  def setSize(size: Int): TypedQueryBuilder[T] =
    this.copy(size = Math.min(size, OpenSearchConstants.MAX_RETURNED_DOCUMENTS))
  ...
  def setSizeToMaximum(): TypedQueryBuilder[T] =
    upgradeToScan().copy(size = OpenSearchConstants.MAX_RETURNED_DOCUMENTS)
  ...
  def bulkRead(i: Int): TypedQueryBuilder[T] =
    this.copy(bulkRead = Math.min(i, OpenSearchConstants.MAX_RETURNED_DOCUMENTS).toInt)
  ...
  def results: ZIO[Any, FrameworkException, SearchResponse] =
    ormManager.openSearchService.search(toRequest)
  ...
  def scan(implicit authContext: AuthContext): ESCursor = {
    val qs = setScan()
    ormManager.openSearchService.searchStream(qs.toRequest)
  }
  ...
  def valueList[R1, R2](
    ...
    ormManager.openSearchService.searchStreamField[R1, R2](queryBuilder.toRequest, field1, field2)
  }
  ...
  def multiGet(ids: Chunk[String]): ZIO[Any, FrameworkException, Chunk[ResultDocument]] = {
    ...
    if (realIndices.isEmpty) ZIO.succeed(Chunk.empty)
      ...
      ormManager.openSearchService.mget(index = realIndices.head, ids = ids).map(_.docs)
    } else
      ormManager.openSearchService
        .mget(
  ...
  def toSourceResultDocument(
    ...
    ormManager.openSearchService.searchStream(this.toRequest)
  }
