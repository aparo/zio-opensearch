...
import zio.opensearch.common.{ ActionRequest, Method, SourceConfig }
import zio.opensearch.common.search.{ SearchRequest, SearchRequestBody }
...
import zio.opensearch.suggestion._
...
trait BaseQueryBuilder extends ActionRequest[SearchRequestBody] {
  ...
  def getRealIndices(indices: Chunk[String]): Chunk[String] =
    indices.map { index =>
      ormManager.openSearchService.concreteIndex(index)
    }
  ...
  def resolveId(name: String, id: String): String =
    if (isSingleIndex) id else s"$name${OpenSearchConstants.SINGLE_STORAGE_SEPARATOR}$id"
