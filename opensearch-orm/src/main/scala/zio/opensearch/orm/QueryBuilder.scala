...
import zio.opensearch.aggregations._
import zio.opensearch.geo.GeoPoint
import zio.opensearch.mappings.RootDocumentMapping
import zio.opensearch.queries.{ BoolQuery, Query }
import zio.opensearch.responses.aggregations.DocCountAggregation
import zio.opensearch.sort.Sort._
import zio.opensearch.sort._
import zio.opensearch.OpenSearchConstants
import zio.json.ast._
import zio.json._
import zio._
import zio.opensearch.common.{ ResultDocument, SourceConfig }
import zio.opensearch.common.search.{ Highlight, HighlightField, SearchRequestBody, SearchResponse }
import zio.opensearch.common.update.UpdateRequest
...
final case class QueryBuilder(
  ...
  def setSizeToMaximum(): QueryBuilder =
    this.copy(size = OpenSearchConstants.MAX_RETURNED_DOCUMENTS)

  def setSize(i: Int): QueryBuilder =
    if (i >= 0)
      this.copy(size = i)
    else if (i >= OpenSearchConstants.MAX_RETURNED_DOCUMENTS)
      this.copy(size = OpenSearchConstants.MAX_RETURNED_DOCUMENTS)
    else this
  ...
  def take(i: Int): QueryBuilder =
    this.copy(size = Math.min(i, OpenSearchConstants.MAX_RETURNED_DOCUMENTS))

  def bulkRead(i: Int): QueryBuilder =
    this.copy(bulkRead = Math.min(i, OpenSearchConstants.MAX_RETURNED_DOCUMENTS))
  ...
  def length: ZIO[Any, FrameworkException, Long] = {
    ...
    ormManager.openSearchService.count(indices = indicesQ, query).map(_.count)
  }
  ...
  def results: ZIO[Any, FrameworkException, SearchResponse] = ormManager.openSearchService.search(toRequest)
  ...
  def extractGroupBy(
    result: Map[String, opensearch.responses.aggregations.Aggregation],
    fields: Chunk[String],
    groupByAggregations: List[GroupByAggregation],
    ...
    if (fields.nonEmpty) {
      ...
      result.flatMap {
        case (name, agg) =>
          if (name.endsWith("_missing") || agg.isInstanceOf[DocCountAggregation]) {
            ...
            agg.asInstanceOf[opensearch.responses.aggregations.BucketAggregation].buckets.flatMap { b =>
              extractGroupBy(
                b.subAggs,
                fields.tail,
                groupByAggregations,
      ...
      groupByAggregations.filter(_.isInstanceOf[Concat]).foreach { co =>
        ...
        val values =
          result(name).asInstanceOf[opensearch.responses.aggregations.BucketAggregation].buckets.map(_.key)
...
//        }
  ...
  def scan: Stream[FrameworkException, ResultDocument] =
    ormManager.openSearchService.searchStream(this.setScan().toRequest)
  ...
  def valueList[R: JsonDecoder](field: String): Stream[FrameworkException, R] = {
    ...
    ormManager.openSearchService.searchStreamField[R](queryBuilder.toRequest, field)
  }
  ...
  def values(fields: String*): Stream[FrameworkException, Json.Obj] = {
    ...
    ormManager.openSearchService.searchStreamFields(queryBuilder.toRequest)
  }
  ...
  def update(doc: Json.Obj, bulk: Boolean = false, refresh: Boolean = false): ZIO[Any, FrameworkException, Int] = {
    ...
    def processUpdate(): ZIO[Any, FrameworkException, Int] =
      scan.map { record =>
        ...
        if (bulk) {
          ormManager.openSearchService.addToBulk(ur).unit
        } else {
          ormManager.openSearchService.update(ur).unit
        }
  ...
  def updateFromDocument(
    ...
    def processUpdate(): ZIO[Any, FrameworkException, Int] =
      scan.map { record =>
        ...
        if (bulk)
          ormManager.openSearchService.addToBulk(ur)
        else ormManager.openSearchService.update(ur)
