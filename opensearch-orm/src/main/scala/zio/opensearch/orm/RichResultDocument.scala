...
import zio.ZIO
import zio.opensearch.common.{ OpType, Refresh, ResultDocument }
import zio.opensearch.common.delete.DeleteResponse
...
object RichResultDocument {
  ...
  implicit class RichResultDocumentImprovements(
    ...
    def delete(bulk: Boolean = false, refresh: Boolean = false)(
      ...
      ormManager.openSearchService.delete(
        doc.index,
        doc.id,
        bulk = bulk,
    ...
    def update(
      ...
      val updateAction =
        UpdateRequest(
          ...
          refresh = Some(zio.opensearch.common.Refresh.fromValue(refresh))
        )
